    ...
    private final class PayUJavaScriptInterface {
        ...
        public void success(long id, final String paymentId) {
            mHandler.post(new Runnable() {
                public void run() {
                    ...
                    Toast.makeText(getApplicationContext(), "Money added to wallet successfully", Toast.LENGTH_LONG).show();
        ...
        public void failure(final String id, final String error) {
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    //cancelPayment();
                    testStatus("", getRechargeAmt, "failure", "Payment cancelled");
        ...
        public void failure(final String params) {
            mHandler.post(new Runnable() {
                ...
                public void run() {
	                    ...
	                    finish();*/
                    testStatus("", getRechargeAmt, "failure", "Transaction failure");
...
//-------------------------------------image slider view----------------------------------------------------------------------
    ...
    public void testStatus(String txnId, String amount, String strStatus, String strDesc) {
        if (strStatus.equals("failure")) {
            ...
            intent.putExtra("test", getFirstName);
            intent.putExtra("txnid", PayMentGateWay.this.params.get("txnid"));
            startActivity(intent);
        }
    }
